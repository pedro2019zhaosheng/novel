...
class libredis
{
    ...
    private function connect(){
        if ( ! $this->connected){
            ...
            try {
                ...
                $this->connect = $this->oRedis->pconnect($this->aServer[0], $this->aServer[1]); //TRUE on success, FALSE on error.
     ...
     */
    public function incr($key, $value=1,$timeout){
        if( ! $this->connect()){
            return false;
        }
     ...
     * @return false/int 返回最新的值
     */
    public function decr($key, $value=1,$timeout){
        if( ! $this->connect()){
            return false;
        }
     ...
     * @return Array
     */
    public function lIndex($key,$index, $zip=false, $serial=true){
        if( ! $this->connect()){
            return array();
        }
     ...
     * @return Boolean
     */
    public function lRem($key, $value, $index, $zip=false, $serial=true){
        if( ! $this->connect()){
            return false;
        }
        $value = $serial ? serialize( $value) : $value;
     ...
     * @return Boolean
     */
     public function sAdd($key, $value, $zip=false, $serial=true,$timeout=0){
         if( ! $this->connect()){
            return false;
        }
        $value = $serial ? serialize( $value) : $value;
     ...
     * @return Mixed/false
     */
    public function sRandMember($key, $zip=false, $serial=true, $num=1){
        if( ! $this->connect()){
            return false;
        }

        $result = [];
        $results = $this->oRedis->sRandMember( $key, $num);
        foreach ($results as $key => $value) {
            $result[] = $zip ? unserialize( @gzuncompress( $value)) : ($serial ? unserialize( $value) : $value);
        }
        return $result;
    }
    /**
     ...
     */
    public function zAdd($key, $score, $value, $zip=false, $serial=false){
        if( ! $this->connect()){
            return array();
        }
        $value = $serial ? serialize( $value) : $value;
     ...
     */
 	public function zRem($key,$index){
        if( ! $this->connect()){
            return array();
        }
        return $this->oRedis->zRem($key, $index);
    }
     ...
     */
    public function zIncrBy($key, $increment, $index){
        if( ! $this->connect()){
            return array();
        }
        return $this->oRedis->zIncrBy($key, $increment, $index);
    }
     ...
     */
    public function hSet($key,$index,$value,$zip=false, $serial=false,$timeout=0){
    	if( ! $this->connect()){
            return array();
        }
     ...
     * @param $key
     */
    public function hGet($key,$index,$zip=false, $serial=false){
        if( ! $this->connect()){
        	return false;
        }

        $result = $this->oRedis->hGet( $key, $index); 
        return $result===false ? $result : ($zip ? unserialize( @gzuncompress( $result)) : ($serial ? unserialize( $result) : $result));
   }
    /**
     * 返回名稱為hashname中的元素個數
