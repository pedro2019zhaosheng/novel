...
(function($, window, undefined) {
	...
	var getTimeMD5 = function() {
		var key = "b55a95e9bf7a8f04";
		var obj = {
			"9": "3c",
			"8": "3d",
			"7": "32",
		...
		var strArr = (new Date().getTime() + "").substr(0, 10).split('');
		var _arr = [];
		for(var i = 0; i < strArr.length; i++) {
			_arr.push(obj[strArr[i]]);
		};
		...
		var leng = parseInt(Math.random() * 100) % 4 + 1;
		return key + _arr.join('') + randomString(leng);
	}
	...
	var ajaxComplete = function(status, xhr, settings) {
		//mui.hideLoading();
		if(xhr.status != 200) {
			mui.toast('请求报错，错误状态' + xhr.status);
		}
...
mui.template = function(text, data) {
	var render;
	var settings = {
		evaluate: /<@([\s\S]+?)@>/g,
		interpolate: /<@=([\s\S]+?)@>/g,
...
mui.getQueryByKey = function(name) { 
	//获取中文会有问题
...
mui.getQueryString = function(strParame) {
	var args = new Object();
...
(function($, window) {
	$.fn.css = function(className, value) {
		if(!$.isObject(className)) {
			if(!value) {
				try {
					return window.getComputedStyle(this[0], null)[className].match(/[\d.]*/)[0]
				} catch(e) {
					return window.getComputedStyle(this[0], null)[className];
				}
			} else {
				this[0].style[className] = value;
			}
		} else {
			for(var i in className) {
				if(/^([+-]?)\d*\.?\d+$/.test(className[i])) {
					this[0].style[i] = className[i] + "px";
				} else {
					this[0].style[i] = className[i];
				}
			}
		}
