...
<div class="panel panel-default">
    <div class="panel-heading">
        <h3 class="panel-title">{{$actview}}
            <a type="button" href="{{url('premission')}}" class="btn btn-default" style="margin-left: 2%"><<返回</a>
        </h3>
    </div>
    <div class="panel-body">
        <form class="form-horizontal" method="post" action="{{url('premission/edit-premission')}}">
            <input type="hidden" value="{{$role->id}}" name="id">
            <div class="form-group">
                <label for="inputEmail3" class="col-sm-2 control-label">权限名称</label>
                <div class="col-sm-4">
                    <input type="text" class="form-control" name="name" value="{{$role->name}}"  placeholder="权限名称">
                </div>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-4">
                    <label class="radio-inline">
                        <input type="radio" name="status" @if($role->status==1) checked @endif  value="1"> 是
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="status" @if($role->status==0) checked @endif  value="0"> 否
                    </label>
                </div>
            </div>
            <div class="form-group">
